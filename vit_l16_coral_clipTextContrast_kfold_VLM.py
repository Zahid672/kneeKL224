"""
ViT-L/16 + CORAL for KL grading on kneeKL224
with CLIP text-guided contrastive + image-feature distillation.

- Student: torchvision ViT-L/16 + CORAL ordinal head.
- Teacher: OpenCLIP ViT-L/14 (frozen).
- Extra losses:
    * Feature distillation: student CLS -> CLIP image features (MSE).
    * Prompt contrastive: student projected features vs KL text prompts
      from CLIP text encoder (cross-entropy over similarity scores).
...
LAMBDA_DISTILL = 0.5
LAMBDA_CONTRAST = 0.2        # a bit higher weight since prompts are richer
CONTRAST_TEMPERATURE = 0.07

METRICS_CSV = "ViT_L16_CORAL_CLIPtext_kfold_metrics.csv"
PREDICTIONS_CSV = "ViT_L16_CORAL_CLIPtext_bestFold_test_predictions.csv"
BEST_MODEL_PATH = "ViT_L16_CORAL_CLIPtext_kfold_bestFold.pth"
...
def get_semantic_prompts() -> List[str]:
    """Option B: semantic medical KL prompts."""
    prompts = [
        "a knee radiograph with no osteoarthritis, normal joint space, no osteophytes, Kellgren Lawrence grade 0",
        "a knee radiograph with very mild osteoarthritis, tiny osteophytes and possible slight joint space narrowing, Kellgren Lawrence grade 1",
        "a knee radiograph with mild osteoarthritis, definite osteophytes and possible joint space narrowing, Kellgren Lawrence grade 2",
        "a knee radiograph with moderate osteoarthritis, multiple osteophytes and definite joint space narrowing and sclerosis, Kellgren Lawrence grade 3",
        "a knee radiograph with severe osteoarthritis, large osteophytes, marked joint space narrowing and bony deformity, Kellgren Lawrence grade 4",
    ]
    return prompts
...
def get_teacher_features(clip_model, images):
    """
    CLIP image features for a batch, with simple two-view aggregation
    (original + horizontal flip) for more robust teacher targets.
    """
    clip_model.eval()
    feats1 = clip_model.encode_image(images)
    feats2 = clip_model.encode_image(torch.flip(images, dims=[3]))
...
def evaluate(
    ...
    for images, labels in loader:
        ...
        preds = model.predict_from_logits(logits, tau=0.5)
...
def run_training():
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(indices, labels), start=1):
        ...
        dummy_in = torch.zeros(1, 3, IMG_SIZE, IMG_SIZE).to(DEVICE)
        with torch.no_grad():
            _, tmp_feat = model(dummy_in)
    ...
    print("\n===== TEST RESULTS (best fold model, no TTA, tau=0.5) =====")
