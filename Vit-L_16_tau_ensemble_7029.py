...
Backbone: ViT-L/16 (IMAGENET1K_V1)
Loss: CORAL (BCE with pos_weight) + per-class sample weights.
Augmentations, weights, and pipeline match the earlier ViT-B/16 setup
that gave you ~0.6926 accuracy, only the backbone is upgraded.
...
BATCH_SIZE = 8          # ViT-L is heavier; increase if GPU memory allows
NUM_EPOCHS = 80
LR = 3e-5
WEIGHT_DECAY = 0.05
...
METRICS_CSV = f"{MODEL_TAG}_kfold_results_kneeKL224_TTA_tau.csv"
PREDICTIONS_CSV = f"{MODEL_TAG}_bestFold_test_predictions_kneeKL224_TTA_tau.csv"
BEST_MODEL_PATH = f"{MODEL_TAG}_bestFold_kneeKL224_TTA_tau.pth"

# ensemble outputs
ENSEMBLE_METRICS_CSV = f"{MODEL_TAG}_ensemble_test_metrics_kneeKL224_TTA_tau.csv"
ENSEMBLE_PREDS_CSV = f"{MODEL_TAG}_ensemble_test_predictions_kneeKL224_TTA_tau.csv"
...
def run_vit_coral_training_and_ensemble():
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(indices, labels_all), start=1):
        ...
        writer = SummaryWriter(log_dir=f"runs/{MODEL_TAG}_kneeKL224/fold_{fold_idx}")
