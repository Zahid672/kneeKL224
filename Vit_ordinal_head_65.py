...
- Stratified K-fold (no data balancing) over train+val.
- Separate held-out test set.
- Image size 224x224 (required by your torchvision ViT).
- Early stopping, cosine LR schedule, TensorBoard.
- Metrics for each fold + test saved to CSV.
...
NUM_FOLDS = 5
IMG_SIZE = 224             # vit_b_16 in your env expects 224x224

BATCH_SIZE = 16
NUM_EPOCHS = 40
LR = 3e-5                  # usually good for ViT fine-tuning
WEIGHT_DECAY = 0.05
...
METRICS_CSV = "ViT_B16_CORAL_kfold_results.csv"
PREDICTIONS_CSV = "ViT_B16_CORAL_bestFold_test_predictions.csv"
BEST_MODEL_PATH = "ViT_B16_CORAL_bestFold.pth"
...
class ViT_CORAL(nn.Module):
    ...
    def __init__(self, num_classes=5, pretrained=True):
        ...
        # ViT backbone; older torchvision still supports `pretrained=`
        try:
    ...
    def predict(self, x):
        ...
        # grade = number of thresholds where p_k >= 0.5
...
def main():
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(
        ...
        writer = SummaryWriter(
            log_dir=f"runs/ViT_B16_CORAL/fold_{fold_idx}"
        )
        ...
        for epoch in range(1, NUM_EPOCHS + 1):
            ...
            train_loss = train_one_epoch(
                model, train_loader, optimizer, bce_loss,
    ...
    # 3) Final test with best fold model
