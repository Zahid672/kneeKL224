...
Stronger settings added:
- Class weights: [1.0, 2.2, 1.4, 1.0, 1.0]
- Stronger train augmentation (ColorJitter + small RandomAffine).
...
METRICS_CSV = "ViT_B16_CORAL_kfold_results_kneeKL224_TTA_tau_strong.csv"
PREDICTIONS_CSV = "ViT_B16_CORAL_bestFold_test_predictions_kneeKL224_TTA_tau_strong.csv"
BEST_MODEL_PATH = "ViT_B16_CORAL_bestFold_kneeKL224_TTA_tau_strong.pth"

# ensemble outputs
ENSEMBLE_METRICS_CSV = "ViT_B16_CORAL_ensemble_test_metrics_kneeKL224_TTA_tau_strong.csv"
ENSEMBLE_PREDS_CSV = "ViT_B16_CORAL_ensemble_test_predictions_kneeKL224_TTA_tau_strong.csv"
...
def run_vit_coral_training_and_ensemble():
    ...
    # Stronger class weights (focus on G1)
    cls_weight_vec = torch.tensor(
        [1.0, 2.2, 1.4, 1.0, 1.0], dtype=torch.float32, device=DEVICE
    )
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(indices, labels_all), start=1):
        ...
        writer = SummaryWriter(log_dir=f"runs/ViT_B16_CORAL_kneeKL224_strong/fold_{fold_idx}")
