"""
Ensemble ordinal models for Knee OA (KL 0..4) with ROI cropping.
...
All use:
  - automatic knee joint ROI crop + CLAHE
  - ordinal head (4 logits for 5 ordered classes)
  - WeightedRandomSampler for train
  - Cosine LR schedule
  - Metrics: Accuracy, macro Precision/Recall/F1, macro Specificity, AUROC(OVR)
  - CSV saving for each model + ensemble.
...
# training mode for each backbone: "IV3" (feature extraction) or "FT" (full fine-tune)
MODE = "FT"  # recommend "FT"
...
def knee_joint_roi_preprocess(pil_img: Image.Image,
    ...
    3) Crop vertical strip centered on that band
    4) Apply CLAHE
    ...
    # Sobel in y -> horizontal edge strength
...
def get_dataloaders():
    input_size = 299  # common size for all backbones
...
def ordinal_probabilities(logits: torch.Tensor, num_classes: int) -> torch.Tensor:
    """
    Convert p_gt[k]=P(y>k) into class probabilities.
...
def build_model(backbone: str, num_thresholds: int, mode: str = "FT") -> nn.Module:
    """
    Build backbone with ordinal head (num_thresholds outputs).
    mode: "IV3"/"FT" -> feature extraction vs full fine-tune
    """
    ...
    if mode == "IV3":  # feature extraction: only ordinal head trainable
        for p in model.parameters():
            p.requires_grad = False
        # enable last linear(s)
...
def train_model(model, dataloaders, dataset_sizes,
                optimizer, scheduler,
                num_epochs=25, device="cpu", backbone_name="model"):
        ...
        for phase in ["train", "val"]:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == "train"):
                    ...
                    # inception returns (main, aux) when aux_logits=True
...
# ---------------------------------------------------------------------
# METRIC COMPUTATION + CSV SAVING
# ---------------------------------------------------------------------

def compute_and_save_metrics(y_true, y_pred, y_score,
                             tag: str, out_dir: str):
    ...
    # CSV: overall metrics
    import csv
    overall_path = os.path.join(out_dir, f"{tag}_overall_metrics.csv")
    ...
    per_class_path = os.path.join(out_dir, f"{tag}_per_class_metrics.csv")
    ...
    cm_path = os.path.join(out_dir, f"{tag}_confusion_matrix.csv")
...
def evaluate_ensemble(models_dict, test_loader, tag="ensemble"):
        ...
        for inputs, labels in test_loader:
            ...
            for name, model in models_dict.items():
                ...
                if isinstance(outputs, tuple):
                    ...
                    logits = outputs
                probs = ordinal_probabilities(logits, NUM_CLASSES)  # [N,K]
                probs_list.append(probs)

            # average probabilities across models
            avg_probs = torch.mean(torch.stack(probs_list, dim=0), dim=0)  # [N,K]
...
if __name__ == "__main__":
    ...
    for backbone in BACKBONES:
        ...
        # LR: slightly higher if only head is trained
        if MODE == "IV3":
            lr = 5e-4
        else:
            lr = 1e-4
        ...
        _ = evaluate_single_model(model, test_loader, tag=backbone)

    # evaluate ensemble
    print("\n========== Evaluating ENSEMBLE ==========")
    _ = evaluate_ensemble(trained_models, test_loader, tag="ensemble")
