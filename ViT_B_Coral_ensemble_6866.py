...
1) 5-fold training + per-fold checkpoints + best-fold TTA test
2) 5-model ensemble (folds 1..5) + TTA test
...
# single best-fold outputs (same as before)
...
# ensemble outputs
ENSEMBLE_METRICS_CSV = "ViT_B16_CORAL_ensemble_test_metrics_kneeKL224_TTA.csv"
ENSEMBLE_PREDS_CSV = "ViT_B16_CORAL_ensemble_test_predictions_kneeKL224_TTA.csv"
...
def ensemble_evaluate_tta(
    ...
    For each batch:
      ...
      - decode CORAL predictions (sigmoid >= 0.5)
    """
...
def run_vit_coral_training_and_ensemble():
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(indices, labels), start=1):
        ...
        # Save fold checkpoint (for ensemble later)
    ...
    test_metrics["split"] = "test_TTA_single"
    ...
    # ---------- ENSEMBLE (5 folds) + TTA ----------
    print("\n===== ENSEMBLE EVALUATION (5-fold, with TTA) =====")
    ...
    y_true_ens, y_pred_ens = ensemble_evaluate_tta(
        models_list, test_loader, DEVICE, NUM_CLASSES, n_augs=4
    )

    ens_metrics = compute_metrics(y_true_ens, y_pred_ens, NUM_CLASSES)
    ens_metrics["fold"] = -1           # indicates ensemble
    ens_metrics["split"] = "test_TTA_ensemble"
    ens_metrics["val_loss"] = 0.0
    print(f"\nEnsemble test accuracy: {ens_metrics['accuracy']:.4f}")
    print(f"Ensemble macro F1: {ens_metrics['f1_macro']:.4f}")
