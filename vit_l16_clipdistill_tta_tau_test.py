"""
ViT-L/16 CORAL + CLIP-distilled (best fold)
TTA + tau tuning for KL grading on kneeKL224.

This script:
  1) Rebuilds the ViT-L/16 CORAL student.
  2) Loads ViT_L16_CORAL_CLIPdistill_bestFold.pth.
  3) Tunes the CORAL threshold tau on the 'val' set.
  4) Evaluates on the 'test' set with TTA + tuned tau.
  5) Prints confusion matrix and classification report and
     saves metrics and predictions to CSV.

Folder structure (same as training):
...
CHECKPOINT = "ViT_L16_CORAL_CLIPdistill_bestFold.pth"
NUM_CLASSES = 5
IMG_SIZE = 224
BATCH_SIZE = 16
NUM_WORKERS = 4
...
TAU_MIN = 0.2
TAU_MAX = 0.8
TAU_STEPS = 61  # step size ~0.01

METRICS_CSV = "ViT_L16_CORAL_CLIPdistill_TTA_tau_metrics.csv"
PREDICTIONS_CSV = "ViT_L16_CORAL_CLIPdistill_TTA_tau_test_predictions.csv"
...
class ViT_L16_CORAL(nn.Module):
    ...
    For inference we only need logits, but we return features as well.
    """
...
def logits_with_tta(model, images, n_augs: int = 4):
    ...
    Returns mean logits over augmentations.
    """
    model.eval()
    augs = [images]
    if n_augs > 1:
...
def collect_probs_and_labels(model, loader):
    ...
    sigmoid(logits) and true labels.
    """
    all_probs = []
    all_labels = []
...
def tune_tau(probs, labels, num_classes: int):
    """
    Grid search tau to maximize macro F1 on validation set.
    probs: [N, K-1] sigmoid outputs for thresholds.
...
def eval_with_tau(model, loader, tau: float):
    """
    Evaluate model with fixed tau and TTA.
    """
    y_true, y_pred = [], []
    for images, labels in loader:
        images = images.to(DEVICE)
...
def main():
    ...
    # load checkpoint (only "model" key)
    state = torch.load(CHECKPOINT, map_location=DEVICE)
    if "model" in state:
        model.load_state_dict(state["model"])
        print(f"Loaded model weights from '{CHECKPOINT}', fold={state.get('fold')}, val_acc={state.get('val_acc')}")
    else:
        model.load_state_dict(state)
        print(f"Loaded model weights from '{CHECKPOINT}' (direct state_dict)")

    val_loader, test_loader = build_loaders()

    # ---------- tau tuning on validation ----------
    print("\nCollecting validation probs with TTA for tau tuning...")
    val_probs, val_labels = collect_probs_and_labels(model, val_loader)
    best_tau, best_val_f1 = tune_tau(val_probs, val_labels, NUM_CLASSES)
    print(f"\nBest tau on validation: {best_tau:.4f} (macro F1 = {best_val_f1:.4f})")

    # ---------- evaluation on test with tuned tau ----------
    ...
    df_metrics = pd.DataFrame([{
        "setting": "ViT_L16_CORAL_CLIPdistill_TTA_tau",
        "tau": best_tau,
        **metrics,
    }])
